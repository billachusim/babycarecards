...
const PROMPT_HINT =
  "Try: “This is Mia, a 14-month-old toddler. She has 6 ounces of formula at 7am, noon and 6pm, naps from 1 to 3, takes half a dropper of her vitamin D drops each morning, and our pediatrician is Green Lane Pediatrics on 555 0134.”";
...
export function VoiceFillDialog({
  ...
  const setPet = (key: keyof ParsedCareDetails["child"], value: string) =>
    setDetails((current) => ({
      ...current,
      child: { ...current.child, [key]: value ? value : null },
    }));
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[85vh] overflow-y-auto rounded-3xl sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-display text-2xl">Talk about your child</DialogTitle>
            <DialogDescription>
          ...
          {stage === "review" && (
            <div className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                  <Label htmlFor="voice-allergies">Allergies</Label>
                  <Input
                    id="voice-allergies"
                    className="mt-1 rounded-xl"
                    value={details.child.allergies ?? ""}
                    onChange={(event) => setPet("allergies", event.target.value)}
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="voice-age"
                    className="mt-1 rounded-xl"
                    value={details.child.approximateAge ?? ""}
          ...
          {stage === "review" && (
            <DialogFooter className="flex-col gap-2 sm:flex-col">
              ...
              {!details.child.name && (
                <p className="text-center text-xs text-muted-foreground">
                  Add your child&apos;s name to continue.
                </p>
              )}
            </DialogFooter>
          )}
...
function ReviewSummary({ details }: { details: ParsedCareDetails }) {
  ...
  push("Snacks", details.feeding.snacks);
  push("Avoid", details.feeding.foodsToAvoid);
  push("Outdoor / walk time", details.routine.outdoorTime);
  push("Playtime", details.routine.playtime);
  push("Sleep", details.routine.sleepRoutine);
  push("Diapering", details.routine.diaperingRoutine);
  push("Emergency contact", details.emergency.primaryName);
  push("Emergency phone", details.emergency.primaryPhone);
  push("Pediatrician", details.pediatrician.doctorName ?? details.pediatrician.clinicName);
  push("Doctor phone", details.pediatrician.phone);
  push("Personality", details.child.personality);
  push("Good to know", details.child.thingsToKnow);
