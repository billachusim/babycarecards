...
import type { ReminderType, RepeatSchedule } from "@/features/children/models";
...
const TYPES: ReminderType[] = ["feeding", "medication", "activity", "diaper", "custom"];
...
function RemindersPage() {
  ...
  const { ready, children, reminders, saveReminder, deleteReminder, toggleReminder, isPremium } =
    useCareStore();
  ...
  const submit = async () => {
    try {
      ...
      if (!childId) throw new Error("Please choose which child this reminder is for.");
      ...
      {ready && children.length === 0 ? (
        <div className="mt-6">
          <EmptyState
            ...
            description="Reminders are attached to a child's care card."
            action={
              <Button className="h-11 rounded-xl" onClick={() => void navigate({ to: "/children/new" })}>
                Add Child
              </Button>
            }
          />
