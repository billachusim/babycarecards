...
export function FeedingStep({ childId }: { childId: string }) {
  const { feedingsFor, saveFeeding, deleteFeeding } = useCareStore();
  const feedings = feedingsFor(childId);

  return (
    <div className="space-y-4">
      {feedings.length === 0 && (
        <p className="rounded-2xl bg-secondary/60 px-4 py-3 text-sm text-muted-foreground">
          Add what your child eats, how much, and when. You can add more than one meal plan.
        </p>
      )}
      ...
      {feedings.map((feeding, index) => (
        <div key={feeding.id} className="space-y-4 rounded-3xl border border-border bg-card p-5">
          ...
          />
          <TextField
            label="Snack instructions"
            optional
            multiline
            value={feeding.snacks ?? ""}
            onChange={(snacks) => saveFeeding({ ...feeding, snacks })}
          />
          <TextField
