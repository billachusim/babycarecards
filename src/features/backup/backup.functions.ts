...
export const pushBackup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const groups: Record<(typeof TABLES)[number], Row[]> = {
      backup_children: own(data.children),
      ...
      backup_pediatricians: own(data.pediatricians),
...
export const pullBackup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const payload: BackupPayload = {
      children: await read("backup_children"),
      ...
      pediatricians: await read("backup_pediatricians"),
