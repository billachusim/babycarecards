...
const UNSAFE_PATTERNS: RegExp[] = [
  ...
  /\bgive (your|the) (dog|cat|child) \d+/i,
];
...
function buildPrompt(existing: Guide[]): string {
  ...
  return `You write practical baby and child-care guides for Baby Care Cards (babycarecards.app), a free tool that turns a child's feeding, routine, medication and emergency details into one care card a parent can hand to a babysitter.

Pick ONE new guide topic and write it in full.

Topic rules:
- It must be a topic a parent or caregiver genuinely searches for: feeding and portions, daily routines, babysitter and grandparent handovers, travel with a baby, newborn/infant/toddler stage-specific care, daycare logistics, emergency preparedness, age-specific routines (newborns, infants, toddlers, and occasionally preschoolers).
...
- For anything health-related, tell the reader to follow their pediatrician's written instructions and set medicalDisclaimer to true.
- Food portion guidance may reference general planning ranges and standard calorie formulas, but must tell the reader to confirm with their pediatrician or the food label.
...
Return ONLY valid JSON in exactly this shape:
{
  ...
  "metaTitle": "SEO title ending with ' — Baby Care Cards', under 70 characters",
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a careful child-care writer. You never give medical, diagnostic or dosing advice. You always return strict JSON with no markdown fences.",
          },
