import type { Reminder } from "@/features/children/models";
...
export const NOTIFICATION_WEB_LIMITS =
  "Browser reminders only fire while Baby Care Cards is open in a tab or window. For anything critical, keep a backup alarm on your phone.";
...
export function showReminderNotification(reminder: Reminder, childName: string): void {
  if (!notificationSupported() || Notification.permission !== "granted") return;
  try {
    new Notification(reminder.title, {
      body: `${childName} · ${formatTime(reminder.time)}`,
