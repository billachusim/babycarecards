...
export const Route = createFileRoute("/care/$childId/qr")({
  head: () => ({
    meta: [
      { name: "robots", content: "noindex, nofollow" },
      { title: "QR code — Baby Care Cards" },
      {
        name: "description",
        content: "Show a QR code so a sitter can open the care card instantly on their phone.",
      },
      { property: "og:title", content: "QR code — Baby Care Cards" },
      { property: "og:description", content: "Scan to open this child's care card." },
    ],
  }),
  component: QrPage,
});
...
function QrPage() {
  ...
  const { ready, getChild, buildCareCard } = useCareStore();
  ...
  }, [ready, child, childId, share.signedIn]);
    ...
    <AppShell>
      ...
      <div className="rounded-3xl border border-border bg-card p-6 text-center">
        <h1 className="font-display text-2xl font-semibold">
          Scan to view {child?.name ?? "this child"}&apos;s Care Card
        </h1>
        <div className="mx-auto mt-6 flex size-64 items-center justify-center rounded-2xl bg-white p-3">
          {qr ? (
            <img src={qr} alt={`QR code linking to ${child?.name ?? "the"} care card`} className="size-full" />
          ) : (
        ...
        <p className="mx-auto mt-5 max-w-sm text-xs leading-relaxed text-muted-foreground">
          The code only contains a link to this card&apos;s address — never your child&apos;s details.
        </p>
        ...
        <div className="mt-6 grid gap-2 sm:grid-cols-2">
          ...
          <Button
            ...
            onClick={() => {
              if (!qr) return;
              downloadDataUrl(qr, `${(child?.name ?? "child").toLowerCase()}-care-card-qr.png`);
